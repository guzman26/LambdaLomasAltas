...
const movePallet = async (palletCode, destination) => {
    ...
    if (!validatePalletCode(palletCode)) {
        return createApiResponse(400, `Código de pallet inválido: ${palletCode}. Debe tener 12 dígitos numéricos.`);
    }
    
    // Validate destination
    const validDestinations = ["TRANSITO", "BODEGA", "VENTA"];
    if (!validDestinations.includes(destination)) {
        return createApiResponse(400, `Destino inválido: ${destination}. Debe ser TRANSITO, BODEGA o VENTA.`);
    }
