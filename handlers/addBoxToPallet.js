...
async function addBoxToPallet(palletId, boxCode) {
  ...
  try {
    ...
    if (parsedFromCodigo.format !== box.formato_caja)
      throw new Error(
        `Format mismatch: pallet=${parsedFromCodigo.format}, box=${box.formato_caja}`
      );
    ...
    try {
      const updateResult = await dynamoDB
        .update({
          ...
          Key: { codigo: palletId },
          UpdateExpression:
            'SET cajas = list_append(if_not_exists(cajas, :empty_list), :newBox), cantidadCajas = cantidadCajas + :increment',
      ...
      if (updateError.code === 'ConditionalCheckFailedException') {
        console.log(
          `Box "${boxCode}" already exists in pallet "${palletId}". Skipping duplicate addition.`
        );
