...
const moveEgg = async (code, destination) => {
    ...
    if (!validateBoxCode(code)) {
        return createApiResponse(400, `Código de caja inválido: ${code}. Debe tener 15 dígitos numéricos.`);
    }
    
    // Validar el destino
    const validDestinations = ["PACKING", "BODEGA", "VENTA", "TRANSITO"];
    if (!validDestinations.includes(destination)) {
        return createApiResponse(400, `Destino inválido: ${destination}. Debe ser PACKING, BODEGA, VENTA o TRANSITO.`);
    }
    ...
    try {
        ...
        if (boxes.length === 0) {
            return createApiResponse(404, `No se encontraron cajas en estado ${currentState} con código ${code}.`);
        }
