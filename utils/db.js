...
const databaseService = {
  ...
  async findAvailablePallet(manufacturingDate, caliber, format) {
    console.log(`🔍 Finding available pallet for date: ${manufacturingDate}, caliber: ${caliber}, format: ${format}`);

    const params = {
      TableName: PALLETS_TABLE,
      FilterExpression: "fechaFabricacion = :fecha AND calibre = :calibre AND formato = :formato AND cantidadCajas < :maxCajas",
      ExpressionAttributeValues: {
  ...
  async createPallet(manufacturingDate, shift, caliber, format) {
    console.log(`➕ Creating new pallet for date: ${manufacturingDate}, shift: ${shift}, caliber: ${caliber}, format: ${format}`);
  ...
  async assignBoxToPallet(boxData, activePalletCode) {
    ...
    const { horario: palletShift, calibre: palletCaliber, formato: palletFormat } = parseFechaCalibreFormato(pallet.fechaCalibreFormato);
    const date = fecha_registro.split("T")[0];
  
    console.log(`📦 Attempting to assign box ${codigo} → pallet with shift: ${palletShift}, caliber: ${palletCaliber}, format: ${palletFormat}`);
  ...
  async updateEggState(code, boxId, newState, location) {
    ...
    const params = {
      TableName: EGG_TABLE,
      Key: { codigo: code },
      UpdateExpression: "SET estado = :state, ubicacion = :location, historialMovimientos = list_append(if_not_exists(historialMovimientos, :empty_list), :movement)",
      ExpressionAttributeValues: {
        ":state": newState,
        ":location": location,
